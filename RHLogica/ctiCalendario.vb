...
Public Class ctiCalendario
    ...
    End Function
    '''''Prestamo
    Public Function datosPrestamo(ByVal idprestamo As Integer) As String()
        Dim dbC As New SqlConnection(StarTconnStrRH)
        dbC.Open()
        Dim cmd As New SqlCommand("SELECT * FROM Prestamo_empleados WHERE idprestamo = @idprestamo", dbC)
        ...
        Return dsP
    End Function
    Public Function agregarPrestamo(ByVal idempleado As Integer, ByVal idsucursal As Integer, ByVal idjornada As Integer, ByVal fecha As String, ByVal observaciones As String, ByVal verificado As Boolean, ByVal nota As String) As String()
        Dim ans() As String

        Dim dbC As New SqlConnection(StarTconnStrRH)
        dbC.Open()
        Dim cmd As New SqlCommand("SELECT idprestamo FROM Prestamo_empleados WHERE fecha = @fecha AND idempleado = @idempleado", dbC)
        ...
        Else
            rdr.Close()
            cmd.CommandText = "INSERT INTO Prestamo_empleados SELECT @idempleado,@idsucursal,@idjornada,@fecha,@observaciones,@verificado,@nota"
            ...
            cmd.ExecuteNonQuery()
            cmd.CommandText = "SELECT idprestamo FROM Prestamo_empleados WHERE idempleado = @idempleado AND fecha = @fecha"
            rdr = cmd.ExecuteReader
            rdr.Read()
            ReDim ans(1)
            ans(0) = "Agregado."
            ans(1) = rdr("idprestamo").ToString
            rdr.Close()
        End If
        ...
        Return ans
    End Function
    Public Function actualizarPrestamo(ByVal idprestamo As Integer, ByVal idempleado As Integer, ByVal idsucursal As Integer, ByVal idjornada As Integer, ByVal fecha As String, ByVal observaciones As String, ByVal verificado As Boolean, ByVal nota As String) As String
        Dim err As String
        Dim dbC As New SqlConnection(StarTconnStrRH)
        dbC.Open()
        Dim cmd As New SqlCommand("UPDATE Prestamo_empleados SET idempleado = @idempleado, idsucursal = @idsucursal, idjornada = @idjornada, fecha = @fecha, observaciones = @observaciones, verificado = @verificado, nota = @nota WHERE idprestamo = @idprestamo", dbC)
        cmd.Parameters.AddWithValue("idprestamo", idprestamo)
        cmd.Parameters.AddWithValue("idempleado", idempleado)
        cmd.Parameters.AddWithValue("idjornada", idjornada)
        cmd.Parameters.AddWithValue("fecha", fecha)
        ...
        Return err
    End Function
    Public Function eliminarPrestamo(ByVal idprestamo As Integer) As String
        Dim dbC As New SqlConnection(StarTconnStrRH)
        dbC.Open()
        Dim err As String
        Dim cmd As New SqlCommand("DELETE FROM Prestamo_empleados WHERE idprestamo = @idprestamo", dbC)
        cmd.Parameters.AddWithValue("idprestamo", idprestamo)
            cmd.ExecuteNonQuery()
            err = "Prestamo eliminado."

        cmd.Dispose() : dbC.Close() : dbC.Dispose()
        Return err
    End Function
    Public Function gvPrestamo(ByVal idempleado As Integer) As DataTable
        ...
        While rdr.Read
            r = dt.NewRow
            r(0) = rdr("idprestamo").ToString
            r(1) = rdr("empleado").ToString
            r(2) = rdr("sucursal").ToString
            r(3) = rdr("jornada").ToString
            r(4) = rdr("fecha").ToString
            r(5) = rdr("observaciones").ToString
            r(6) = rdr("verificado").ToString
            dt.Rows.Add(r)
        End While
        rdr.Close() : rdr = Nothing : cmd.Dispose() : dbC.Close() : dbC.Dispose()
        Return dt
    ...
    Public Function gvPrestamoCHK(ByVal idsucursal As Integer, ByVal FechaIn As String, ByVal FechaFn As String) As DataTable
        ...
        Return dt
    End Function
    Public Function actualizarPrestamoCHK(ByVal idprestamo As Integer, ByVal verificado As Boolean) As String
        Dim err As String
        Dim dbC As New SqlConnection(StarTconnStrRH)
        dbC.Open()
        Dim cmd As New SqlCommand("UPDATE Prestamo_empleados SET verificado = @verificado WHERE idprestamo = @idprestamo", dbC)
        cmd.Parameters.AddWithValue("idprestamo", idprestamo)
        cmd.Parameters.AddWithValue("verificado", verificado)
        cmd.ExecuteNonQuery()
        err = "Datos actualizados."
    ...
    Public Function gvParticulares(ByVal idempleado As Integer) As DataTable
        ...
        While rdr.Read
            r = dt.NewRow
            r(0) = rdr("idparticulares").ToString : r(1) = rdr("tipo").ToString : r(2) = rdr("fecha").ToString : r(3) = rdr("observaciones").ToString : r(4) = rdr("cantidad").ToString : r(5) = rdr("verificado").ToString
            dt.Rows.Add(r)
        End While
        rdr.Close() : rdr = Nothing : cmd.Dispose() : dbC.Close() : dbC.Dispose()
        Return dt
    ...
    Public Function datosParticulares(ByVal idparticulares As Integer) As String()
        ...
        If rdr.Read Then
            ...
            dsP(4) = rdr("observaciones").ToString
            dsP(5) = rdr("cantidad").ToString
            dsP(6) = rdr("verificado").ToString
            dsP(7) = rdr("nota").ToString
        Else
            ReDim dsP(1) : dsP(1) = "Error: no se encuentra."
        End If
        rdr.Close() : rdr = Nothing : cmd.Dispose() : dbC.Close() : dbC.Dispose()
        Return dsP
    End Function
    Public Function agregarParticulares(ByVal idempleado As Integer, ByVal tipo As String, ByVal fecha As String, ByVal observaciones As String, ByRef cantidad As Integer, ByVal verificado As Boolean, ByVal nota As String) As String()
        Dim ans() As String
        If tipo <> "" Then
            Dim dbC As New SqlConnection(StarTconnStrRH)
            dbC.Open()
            ...
            Else
                rdr.Close()
                ' cmd.CommandText = "INSERT INTO Particulares SELECT @idempleado,@tipo,@fecha,@observaciones,@cantidad"
                cmd.CommandText = "INSERT INTO Particulares (idempleado,tipo,fecha,observaciones,cantidad,verificado,nota) 
                    values('" & idempleado & "','" & tipo & "','" & fecha & "','" & observaciones & "','" & cantidad & "','" & verificado & "','" & nota & "')"
        ...
        Return ans
    End Function
    Public Function actualizarParticulares(ByVal idparticulares As Integer, ByVal idempleado As Integer, ByVal tipo As String, ByVal fecha As String, ByVal observaciones As String, ByVal cantidad As Integer, ByVal verificado As Boolean, ByVal nota As String) As String
        Dim err As String
        If tipo = "" Then
            err = "Error: no se actualizó, es necesario capturar"
        Else
            ...
            Dim cmd As New SqlCommand("UPDATE Particulares SET tipo = @tipo, fecha = @fecha, observaciones = @observaciones ,cantidad = @cantidad,verificado = @verificado, nota = @nota  WHERE idparticulares = @idparticulares", dbC)
            cmd.Parameters.AddWithValue("tipo", tipo)
            cmd.Parameters.AddWithValue("fecha", fecha)
            cmd.Parameters.AddWithValue("idempleado", idempleado)
            cmd.Parameters.AddWithValue("observaciones", observaciones)
    ...
    Public Function eliminarParticulares(ByVal idparticulares As Integer) As String
        Dim dbC As New SqlConnection(StarTconnStrRH)
        dbC.Open()
        Dim cmd As New SqlCommand("DELETE FROM Particulares WHERE idparticulares = @idparticulares", dbC)
        cmd.Parameters.AddWithValue("idparticulares", idparticulares)
        cmd.ExecuteNonQuery()
    ...
    Public Function gvParticularesCHK(ByVal idsucursal As Integer, ByVal FechaIn As String, ByVal FechaFn As String) As DataTable
        ...
        While rdr.Read
            r = dt.NewRow
            r(0) = rdr("idparticulares").ToString : r(1) = rdr("empleado").ToString : r(2) = rdr("tipo").ToString : r(3) = rdr("fecha").ToString : r(4) = rdr("observaciones").ToString : r(5) = rdr("cantidad").ToString : r(6) = rdr("verificado").ToString
            dt.Rows.Add(r)
        End While
        rdr.Close() : rdr = Nothing : cmd.Dispose() : dbC.Close() : dbC.Dispose()
        Return dt
    End Function
    Public Function actualizarParticularesCHK(ByVal idparticulares As Integer, ByVal verificado As Boolean) As String
        Dim err As String

        Dim dbC As New SqlConnection(StarTconnStrRH)
        dbC.Open()

        Dim cmd As New SqlCommand("UPDATE Particulares SET verificado = @verificado  WHERE idparticulares = @idparticulares", dbC)
